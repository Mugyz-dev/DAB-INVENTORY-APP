  ...
  return (
    <div>
        ...
        <div className="row g-2 align-items-end">
          <div className="col-md-4">
            ...
            <select className="form-control" required value={form.product_id}
              ...
              {products.map(p => <option key={p.id} value={p.id}>{p.sku} - {p.name} (stock: {fmtQty(p.quantity, p.unit)})</option>)}
            </select>
          </div>
          <div className="col-md-2">
            ...
            <select className="form-control" value={form.movement_type}
              ...
              <option value="OUT">Waste / Stock OUT</option>
              <option value="ADJUST">ADJUST (set value)</option>
            </select>
          </div>
          <div className="col-md-2">
            <label className="form-label small mb-0">Quantity / weight</label>
            <input type="number" min="0.001" step="0.001" className="form-control" value={form.quantity}
              onChange={e => setForm({ ...form, quantity: e.target.value })} />
          </div>
          <div className="col-md-3">
            <label className="form-label small mb-0">Note</label>
            <input className="form-control" placeholder="Purchase, spoilage, trimming, correction"
      ...
      <table className="table table-sm table-striped bg-white shadow-sm">
        <thead><tr><th>Date</th><th>Meat item</th><th>Type</th><th className="text-end">Qty</th><th>Note</th><th>User</th></tr></thead>
        <tbody>
          {list.map(m => (
            <tr key={m.id}>
              ...
              <td className="text-end">{fmtQty(m.quantity, m.unit)}</td>
